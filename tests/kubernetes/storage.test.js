...
describe('Kubernetes Storage Configuration', () => {
  ...
  describe('Volume Mounts', () => {
    test('Backend pods have correct volume mounts', async () => {
      ...
      const container = deployment.body.spec.template.spec.containers.find(c => c.name === 'backend');
  ...
  describe('Data Persistence', () => {
    test('PostgreSQL data persists across pod restarts', async () => {
      ...
      if (pods.body.items.length > 0) {
        ...
        await execAsync(
          `kubectl exec -n ${namespace} ${pod.metadata.name} -- psql -U ytempire_user -d ytempire_dev -c "INSERT INTO persistence_test (data) VALUES ('test-data-${Date.now()}')"`
        );
