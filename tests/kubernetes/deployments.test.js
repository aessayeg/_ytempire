...
describe('Kubernetes Deployments and Pods', () => {
  ...
  describe('Deployments Status', () => {
    test('All deployments are running successfully', async () => {
      ...
      deployments.forEach((deploymentName) => {
        ...
        expect(deployment.status.conditions.find((c) => c.type === 'Available').status).toBe(
          'True'
        );
      });
    });
    ...
    test('Deployment replicas match specifications', async () => {
      const backendDeploy = await k8sAppsApi.readNamespacedDeployment(
        'ytempire-backend',
        namespace
      );
      ...
      const frontendDeploy = await k8sAppsApi.readNamespacedDeployment(
        'ytempire-frontend',
        namespace
      );
  ...
  describe('Liveness and Readiness Probes', () => {
    test('All pods pass liveness probes', async () => {
      ...
      const livenessFailures = events.filter(
        (e) => e.reason === 'Unhealthy' && e.message.includes('Liveness probe failed')
      );
    ...
    test('Probe configurations are correct', async () => {
      ...
      const backendContainer = backendDeploy.body.spec.template.spec.containers.find(
        (c) => c.name === 'backend'
      );
  ...
  describe('Resource Limits and Requests', () => {
    ...
    test('Resource allocations are within cluster capacity', async () => {
      const { stdout } = await execAsync('kubectl top nodes -o json').catch(() => ({
        stdout: '{}',
      }));
      ...
      if (stdout !== '{}') {
        ...
        nodeMetrics.items.forEach((node) => {
          ...
          expect(memoryUsage).toBeLessThan(
            (parseInt(node.allocatable.memory.replace('Ki', '')) / 1024 / 1024) * 0.8
          );
        });
      }
    });
