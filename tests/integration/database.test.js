...
describe('Database Integration', () => {
  ...
  test('Connection pooling works correctly', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      promises.push(
        pgClient.query('SELECT pg_sleep(0.1), $1::int as num', [i])
      );
    }
  ...
  test('Database backup and restore simulation', async () => {
    ...
    const restoreResult = await pgClient.query(
      ...
      [backupData.email + '-restored', backupData.username + '-restored', backupData.password_hash, backupData.first_name, backupData.last_name]
    );
