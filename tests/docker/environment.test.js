...
describe('Environment Configuration', () => {
  test('All required environment variables are loaded in backend', async () => {
    ...
    expect(env.DATABASE_URL).toContain(
      'postgresql://ytempire_user:ytempire_pass@postgresql:5432/ytempire_dev'
    );
  ...
  test('YouTube API configuration is present', async () => {
    const { stdout } = await execAsync('docker-compose exec -T backend printenv | grep YOUTUBE');
  ...
  test('Email configuration points to MailHog', async () => {
    const { stdout } = await execAsync('docker-compose exec -T backend printenv | grep SMTP');
