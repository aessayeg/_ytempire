...
describe('Docker Compose Services', () => {
  ...
  test('Frontend service is accessible on port 3000', async () => {
  ...
  test('Backend service is accessible on port 5000', async () => {
    const isReady = await waitForService('backend', `${SERVICES.backend.url}${SERVICES.backend.healthEndpoint}`);
    expect(isReady).toBe(true);
  ...
  test('PostgreSQL is accessible on port 5432', async () => {
  ...
  test('Redis is accessible on port 6379', async () => {
  ...
  test('Nginx proxy routes correctly', async () => {
    const isReady = await waitForService('nginx', SERVICES.nginx.url);
    expect(isReady).toBe(true);
    
    // Test health endpoint
    const healthResponse = await axios.get(`${SERVICES.nginx.url}${SERVICES.nginx.healthEndpoint}`);
  ...
  test('pgAdmin is accessible on port 8080', async () => {
    const isReady = await waitForService('pgadmin', SERVICES.pgadmin.url);
    expect(isReady).toBe(true);
  }, RETRY_OPTIONS.timeout);

  test('MailHog UI is accessible on port 8025', async () => {
